...
export class TracksService {
  ...
  async findAll(filterDto: FilterTracksDto = {}) {
    ...
    const queryBuilder = this.tracksRepository
      ...
      .leftJoinAndSelect('track.reviews', 'review', 'review.isVisible = :visible', {
        visible: true,
      })
      .select([
        'track',
    ...
    if (search) {
      queryBuilder.andWhere(
        '(track.title ILIKE :search OR track.description ILIKE :search OR producer.displayName ILIKE :search OR EXISTS (SELECT 1 FROM unnest(track.tags) AS tag WHERE tag ILIKE :search))',
        { search: `%${search}%` },
      );
    }
