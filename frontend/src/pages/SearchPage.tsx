...
import { X, SlidersHorizontal, Play, Pause, Music, Heart, ShoppingCart, Edit2, Trash2 } from 'lucide-react'
...
const SearchPage = () => {
  ...
  // Load tracks function with useCallback to prevent infinite loops
  const loadTracks = useCallback(async () => {
    ...
    try {
      ...
      // Handle multiple categories - need to filter client-side if backend doesn't support
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2 flex-wrap">
                {searchQuery && (
                  <span className="px-3 py-1 bg-gray-800 border border-gray-700 rounded-full text-sm flex items-center gap-2">
                    Поиск: {searchQuery}
                    <button
                ...
                {activeFiltersCount > 0 && (
                  <button
                    onClick={handleClearFilters}
                    className="px-3 py-1 bg-gray-800 border border-gray-700 rounded-full text-sm hover:bg-gray-700 transition-colors"
                  >
                    Очистить фильтры
                  </button>
                )}
                        ...
                        {currentUser && (track.producerId === currentUser.id || currentUser.role === 'admin') && (
                          <div className="absolute top-1.5 right-1.5 flex items-center space-x-1 z-30">
                            <button
                              ...
                              className="bg-white bg-opacity-90 hover:bg-opacity-100 rounded-full p-1.5 transition-all shadow-md"
