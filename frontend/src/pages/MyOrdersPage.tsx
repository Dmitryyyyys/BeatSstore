...
import { MessageSquare, Users, Heart, Bookmark, Clock, LogOut, Settings, User, Search, Music, Edit2, Trash2, Play, Pause, ShoppingBag } from 'lucide-react'
...
const MyOrdersPage = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        console.error('Error initializing MyOrdersPage:', error)
      }
    }
  ...
  const loadPurchases = async () => {
    try {
      ...
      setPurchases([]) // Set empty array on error to prevent white screen
    } finally {
      setLoading(false)
    }
  }
  ...
  try {
    filteredPurchases = (purchases || []).filter(
      (item) => {
        ...
        try {
          ...
          console.error('Error filtering purchase item:', err, item)
          return false
        }
      }
    )
    ...
    console.error('Error filtering/sorting purchases:', error)
    filteredPurchases = []
  }
          ...
          {filteredPurchases.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-300">
                {searchQuery ? 'Ничего не найдено' : 'У вас пока нет покупок'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {filteredPurchases.map((purchase) => {
                const isTrackPlaying = currentTrack?.id === purchase.track.id && isPlaying
                return (
                  <div
                    key={purchase.id}
                  ...
                  >
                    <Link to={`/tracks/${purchase.track.id}`}>
                      <div className="relative aspect-square bg-gradient-to-br from-primary-400 to-purple-600 group cursor-pointer">
                        ...
                        {purchase.track.bpm && (
                          <div className="absolute top-1.5 left-1.5 bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded z-10">
                            {purchase.track.bpm} BPM
                          </div>
                        )}

                        {/* Play Button Overlay */}
                        {(purchase.track.previewUrl || purchase.track.fileUrl) && (
                        ...
                        {currentUser && purchase.track.producerId === currentUser.id && (
                          <div className="absolute top-1.5 right-1.5 flex items-center space-x-1 z-20">
                            <Link
                              to={`/tracks/${purchase.track.id}/edit`}
                            ...
                            <button
                              onClick={(e) => {
                                e.preventDefault()
                                e.stopPropagation()
                                handleDeleteClick(purchase.track.id, purchase.track.title)
                              }}
                    ...
                    <div className="p-2.5">
                      ...
                      <p className="text-xs text-gray-300 mb-2 truncate">
                        {purchase.track.producer?.displayName || 'Unknown Producer'}
                      </p>
