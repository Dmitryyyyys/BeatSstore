...
import { Play, Pause, Save, X, ChevronDown, Radio, Zap, Settings2, ChevronUp, Upload, Trash2, Download, Music, Plus, Volume2, RotateCcw, Square } from 'lucide-react'
...
interface SerializedSampleFile {
  fileName: string
  dataUrl: string
}

interface BeatProjectData {
  version: number
  pattern: 'A' | 'B'
  bpm: number
  steps: { [key: string]: boolean }
  instrumentParams: DefaultParams
  sampleFiles?: Record<string, SerializedSampleFile>
}
...
const BeatSequencer = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.bpm-dropdown')) setShowBpmDropdown(false)
      if (!target.closest('.save-dropdown')) setShowSaveDropdown(false)
    }
    const handleMouseUp = () => { dragRef.current.active = false }
    const handleKeyDown = (e: KeyboardEvent) => { if (e.key === 'Escape') setSelectedCells(new Set()) }
  ...
  const restoreSampleFile = useCallback(async (instrumentId: number, sampleFile: SerializedSampleFile) => {
    if (!audioContext) throw new Error('Audio context is not ready')
    const response = await fetch(sampleFile.dataUrl)
    const arrayBuffer = await response.arrayBuffer()
    const audioBuffer = await audioContext.decodeAudioData(arrayBuffer.slice(0))
    setLoadedSamples((prev) => ({ ...prev, [instrumentId]: audioBuffer }))
    setSampleFileNames((prev) => ({ ...prev, [instrumentId]: sampleFile.fileName }))
  }, [audioContext])
  ...
  const scheduleInstrumentPlayback = useCallback((
    ...
    if (loadedSamples[instrumentId]) {
      ...
      if (sliceDuration && stepIndex !== undefined) {
        const offset = stepIndex * sliceDuration
        const playDur = Math.min(sliceDuration, sampleBuffer.duration - offset)
        if (offset < sampleBuffer.duration && playDur > 0) source.start(startTime, offset, playDur)
      } else {
        source.start(startTime)
        source.stop(startTime + sampleBuffer.duration)
      }
      trackSource(source)
      return
    }
  ...
  const scheduleImportedTrackPlayback = useCallback((
    ...
    const offset = track.sliceOffsets[stepIndex] ?? stepIndex * track.sliceDuration
    const playDur = track.sliceDurations[stepIndex] ?? Math.min(track.sliceDuration, track.audioBuffer.duration - offset)
    if (offset >= track.audioBuffer.duration || playDur <= 0) return
  ...
  const playStep = useCallback((step: number, preciseTime?: number) => {
    ...
    instruments.forEach((instrument) => {
      if (steps[`${instrument.id}-${step}`] && isInstrumentAudible(instrument.id)) {
        scheduleInstrumentPlayback(audioContext, audioContext.destination, instrument.id, t, step, activeSourcesRef.current)
      }
    })
  ...
  const startTransport = useCallback((target: string | null = null) => {
    ...
    playStep(0, playStartTimeRef.current)
  ...
  }, [audioContext, bpm, playStep, stopAllSources, totalSteps])
  ...
  const renderBeatToAudioBuffer = useCallback(async () => {
    ...
    for (let stepIndex = 0; stepIndex < totalSteps; stepIndex++) {
      ...
      instruments.forEach((instrument) => {
        if (steps[`${instrument.id}-${stepIndex}`]) {
          scheduleInstrumentPlayback(offlineContext, offlineContext.destination, instrument.id, startTime, stepIndex)
        }
      })
      importedTracks.forEach((track) => {
  ...
  const handleImportProject = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const parsed = JSON.parse(await readFileAsText(file)) as Partial<BeatProjectData>
      stopPlayback()
      setPattern(parsed.pattern === 'B' ? 'B' : 'A')
      setBpm(typeof parsed.bpm === 'number' ? parsed.bpm : DEFAULT_BPM)
      setSteps(parsed.steps ?? {}); setInstrumentParams(mergeInstrumentParams(parsed.instrumentParams))
      setLoadedSamples({}); setSampleFileNames({})
      for (const [id, sf] of Object.entries(parsed.sampleFiles ?? {})) {
        if (sf?.dataUrl && sf?.fileName) await restoreSampleFile(Number(id), sf)
      }
    } catch { alert('Не удалось загрузить проект.') }
    finally { event.target.value = ''; setIsImportingProject(false) }
  }, [restoreSampleFile, stopPlayback])
  ...
  const bpmOptions = [80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180]
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
          ...
          <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
            <div className="flex items-center space-x-4">
              ...
              <div className="relative bpm-dropdown">
                <button onClick={() => setShowBpmDropdown(!showBpmDropdown)} className="flex items-center space-x-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors">
                  <span>BPM: {bpm} | {totalSteps} шагов</span>
                  <ChevronDown className="h-4 w-4" />
                </button>
                {showBpmDropdown && (
                  <div className="absolute top-full left-0 mt-2 bg-gray-700 rounded-lg shadow-lg z-10 min-w-[120px] max-h-60 overflow-y-auto">
                    {bpmOptions.map((o) => (
                      <button key={o} onClick={() => { setBpm(o); setShowBpmDropdown(false) }} className={`w-full text-left px-4 py-2 hover:bg-gray-600 first:rounded-t-lg last:rounded-b-lg ${o === bpm ? 'bg-gray-600' : ''}`}>{o}</button>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center space-x-3 flex-wrap gap-2">
              ...
              <input ref={projectFileInputRef} type="file" accept=".json,application/json" onChange={handleImportProject} className="hidden" />
              <button onClick={() => projectFileInputRef.current?.click()} disabled={isImportingProject} className="flex items-center space-x-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors disabled:opacity-50">
                <Upload className="h-4 w-4" /><span>{isImportingProject ? 'Загрузка...' : 'Проект'}</span>
              </button>
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div className="flex-shrink-0 w-32">
                ...
                {instruments.map((instrument) => (
                  <div key={instrument.id} className="h-16 flex items-center justify-center border-r border-gray-700 group relative" title={instrument.name}>
                    <div className="text-center">
                      ...
                      <div className="mb-1 flex items-center justify-center text-white">{instrument.icon}</div>
                      <div className="text-[10px] text-gray-500 mt-0.5">
                        {sampleFileNames[instrument.id]
                          ? <span className="text-green-400 truncate max-w-[80px] inline-block" title={sampleFileNames[instrument.id]}>{sampleFileNames[instrument.id].length > 8 ? sampleFileNames[instrument.id].slice(0, 6) + '…' : sampleFileNames[instrument.id]}</span>
              ...
              <div className="flex-1 min-w-0 overflow-x-auto">
                ...
                <div className="flex gap-1 mb-2" style={{ width: `${totalSteps * 50}px` }}>
                  {Array.from({ length: totalSteps }).map((_, si) => (
                    <div key={si} className={`text-center text-xs py-2 flex-shrink-0 transition-colors ${currentStep === si && isPlaying ? 'font-bold text-primary-400 bg-primary-900/30 rounded' : si % 4 === 0 ? 'font-bold text-white' : 'text-gray-400'}`} style={{ width: '48px' }}>{si + 1}</div>
                  ))}
                </div>
                {/* Imported track rows */}
                {importedTracks.map((track) => (
          ...
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-700">
            ...
            <div className="flex items-center space-x-2">
              {selectedCells.size > 0 && <span className="text-xs text-cyan-400 mr-2">Выделено: {selectedCells.size} | ЛКМ выделяет, ПКМ активирует | Esc сбросить</span>}
